    ...
    public class DatingRepository : IDatingRepository
    {
        ...
        public async Task<Like> GetLike(int userId, int recipentId)
        {
            return await _context.Likes
                .FirstOrDefaultAsync(u => u.LikerId == userId && u.LikeeId == recipentId);
        }

        public async Task<Photo> GetMainPhotoForUser(int userId)
        {
            return await _context.Photos.Where(u=>u.UserId==userId)
            .FirstOrDefaultAsync(c=>c.IsMain);
        ...
        public async Task<PagedList<User>> GetUsers(UserParams userParams)
        {
            ...
            if(userParams.MinAge!=18 || userParams.MaxAge != 99)
            {
                ...
                var maxDom = DateTime.Today.AddYears(-userParams.MinAge);

                users = users.Where(u => u.DateOfBirth >= minDOB && u.DateOfBirth <= maxDom);
            }
